...
## 🔄 2025-11-17 セッション引継ぎ（再起動前）

### 実施済み
- コミット 397f62c をプッシュ済み（Netlify自動デプロイ中）
- firebase-config.js: `window.currentFirebaseUser = user;` 追加
- index-kanban.html: `FirebaseDB.getUsers()` → `getActiveUsers()` 変更

### 次回セッションで実行すべきこと
1. テスト再実行: `npm run test -- tests/hidden-task-all-users.spec.js`
2. テスト結果確認（特に日本語名取得とログイン成功）
3. 失敗時は `getCurrentUser()` の実装を再確認
4. パスワードが正しいか Firebase Authentication で確認

### 調査が必要な問題
- `getCurrentUser()` が日本語名を返さない（`"muranaka-tenma"` instead of `"邨中天真"`）
- 橋本以降のユーザーでログイン失敗

---

### 認証情報（テスト用）
- **開発者 (muranaka-tenma)**: Tenma7041
- **それ以外全員**: aikakumei

テストファイル `tests/hidden-task-all-users.spec.js` のパスワードを修正済み。
