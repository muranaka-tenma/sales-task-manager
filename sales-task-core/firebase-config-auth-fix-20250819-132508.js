    ...
    if (user) {
        ...
        try {
            ...
            if (userDoc.exists()) {
                ...
                // Firestoreにユーザー情報がない場合はハードコードマップをフォールバック
                console.warn('⚠️ [AUTH] Firestoreにユーザー情報なし。フォールバック使用:', user.email);
        ...
        const sessionData = {
            user: {
                ...
                name: displayName,
                email: user.email,
                role: userRole // 🔥 Firestoreから取得したrole
            },
...
// セッション管理 - Firebase専用（同期版：LocalStorageから取得）
window.getCurrentUser = function() {
    try {
        // 🔥 LocalStorageのcurrentSessionから取得（Firebaseが設定）
